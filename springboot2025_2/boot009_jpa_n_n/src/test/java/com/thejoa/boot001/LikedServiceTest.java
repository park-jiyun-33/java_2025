...
public class LikedServiceTest {
  ...
  public void  testlikePost() {
      ...
      assertTrue(updatedUser.getLikedPosts().contains(post)); //false

	  // 게시글의 likedUsers에서 사용자추가확인
      Post updatedPost = postRepository.findById(post.getId()).orElseThrow();
      assertTrue(updatedPost.getLikedUsers().contains(user)); //false
      //import static org.junit.jupiter.api.Assertions.assertTrue;
  }
  ...
  @Transactional
  @Commit
  public void  testgetLikeCount() {  // 좋아요 갯수
	  	System.out.println(likedService.getLikeCount(8L));  // 있는번호 확인
