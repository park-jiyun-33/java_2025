...
public class BoardService  {
	...
	/*  PAGING  */
	/*  PAGING  */
	// 1 : 최신글 10개		order by bno desc limit 0, 10   0번째부터 10개
	// 2 : 그다음최신글 10개	order by bno desc limit 10, 10  10번쨰부터 10개
	...
	public int update(MultipartFile file, Board board) {
		...
		if(file.getOriginalFilename().length() != 0) { // 2. 새로운 파일이 올라왔는지 확인.
			try {
				save = upload.fileUpload(file);  // 업로드기능
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		board.setBfile(save);  // 3. 파일이름 저장
