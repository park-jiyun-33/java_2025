...
public class BoardController {
	 ...
	 * model.addAttribute("list" , service.findAll()); //##전체리스트뽑고 return
	 * "board/list"; // board 폴더안에 / list파일 } // http://localhost:8080/board/list
	 */
	...
	public String update_post(MultipartFile file, Board board, RedirectAttributes rttr) {
		String msg = "fail";
		if (service.update(file, board) > 0) {
			msg = "글 수정완료!";
		}
...
/*
Restful Api detail?bno=10 쿼리스트링 Restful Api :
localhost:8080/board/detail/bno/10 쿼리스트링 1) resultful - http url을 기반으로 자원에
접근해서 서비스를 제공하는 애플리케이션 2) api 3) method ( GET : 검색 / POST : 생성 / PATCH(부분업데이트), PUT(전체업데이트) / DELETE(삭제) ) 4) DATA - json, xml
*/
